...
/**/
	...
	$( '#pp_15_' + counter ).children( 'button' ).each( function( index ){
		if( $( '#pp_18_' + counter ).html().trim() != '' ){
			id = $( '#pp_18_' + counter ).html().trim();
			alert( "El registro tiene medidas que dependen de el, para eliminar es necesario asignarlo a un proveedor - producto!" );
			reassign = true;
			return false;
		}
	});
...
//			parseFloat(Math.round(278.6 * 100) / 100).toFixed(2)
	...
	function validateNoRepeatBarcode( obj ){
		...
		if( value != '' ){
			...
			$( '#product_provider_list tr' ).each(function( index ){
				$( this ).children( 'td' ).each( function( index2 ){
					if( $( this ).html().trim() == value && $( this ).html().trim() != '' ){
						$( obj ).val( '' );
						exists = true;
