	...
	if( isset( $_GET['fl'] ) ){
		...
		switch ( $action ) {
			...
			case 'validateBarcodesSeriesUpdate' : 
				echo validateBarcodesSeriesUpdate( $link );
			break;
...
//die( $sql );
	...
	function validateBarcodesSeriesUpdate( $link ){
		$sql = "SELECT 
		...
		if( $row['last_update'] < $row['current_year'] ){
			$resp = "<div class=\"row\">";
				$resp .= "<div class=\"col-2\"></div>";
	...
	function updateBarcodesPrefix( $link, $no_reponse = null ){
