	...
	<div id="contenido">
		...
		/*Modificacion Oscar 22.02.2018*/
				...
				$sql="SELECT 
							/*0*/p.id_productos,
							/*1*/p.nombre,
							/*2*/IF(md.id_producto IS NULL,0,IF(SUM(md.cantidad*tm.afecta) IS NULL,0,SUM(md.cantidad*tm.afecta))),
							/*3*/p.orden_lista,
		...
		//declaramos contador
			...
			//consulta detalle de inventario por proveedor producto¡
				$sql_prov_prod="SELECT 
						/*0*/p.id_productos,
						/*1*/CONCAT(p.nombre, IF( pp.id_proveedor_producto IS NULL, '', CONCAT( ' - ', pp.clave_proveedor ) ) ) AS nombre,
						/*2*/SUM( IF( mdp.id_movimiento_almacen_detalle IS NULL, 0, ( mdp.cantidad * tm.afecta) ) ) AS inventario,
						/*3*/p.orden_lista,
						/*4*/IF($sucursal=1,p.ubicacion_almacen,sp.ubicacion_almacen_sucursal),
