<?php
	...
	switch ( $action ) {
		...
		case 'getProductProviderMeasures':
			echo getProductProviderMeasures( $_GET['product_provider_id'], $_GET['reception_detail_id'], $_GET['imgs_path'], $link );
		break; 
	...
	function reassignMeassure( $id, $product_provider_id, $link ){
		...
		$stm = $link->query( $sql ) or die( "Error al actualizar el registro de medidas temporales : {$link->error}" );
...
//return $sql;
			...
			//recupera el id insertado
				...
				for( $i = 9; $i <= 11; $i ++ ){
					...
					$newLocation = "../../../../../files/packs_img/{$meassure[$i]}";
	...
	//						$resp .= "<th>Imágenes</th>";
			...
			//echo "<p>Product_provider {$product_provider_id}</p>";
				...
				while ( $row = $stm->fetch_assoc() ) {
					$resp .= buildMeassureRow( $row, $counter, $imgs_path . 'packs_img/', false, $link );
					$counter ++;
				}
				echo $sql;
			}
			//if( $reception_detail_id != 'null' ){
				...
				while ( $row = $stm->fetch_assoc() ) {
					$resp .= buildMeassureRow( $row, $counter, $imgs_path . 'packs_img_tmp/', true, $link );
					$counter ++;
				}
	...
	//	die('here');
		$resp = "";
		$combo = getComboPackBags( $counter, $row['pack_bag_id'], $link );
	...
	function getComboPackBags( $counter, $pack_bag_id, $link ){
		$sql= "SELECT 
	...
	function build_table_product_provider( $id, $current_count, $reception_detail_id, $path, $link ){
		...
		$resp .= '<thead>';
			$resp .= '<tr><th colspan="15" style="font-size : 200%; color : gray;">' . $product_nam . '</tr>';
			$resp .= '<tr>';
