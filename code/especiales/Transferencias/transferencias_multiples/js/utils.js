...
/*Funciones de asignación de partidas de transferencias a empleados*/
	...
	function seek_people_loged( show_all = false ){
		var txt = $( '#people_seeker' ).val().trim();
		if( txt.length <= 2 && show_all == false ){
			$( '.people_seeker_response' ).css( 'display', 'none' );
			return false;
		}
		var response = ajaxR( "php/db.php?fl=seekPeopleLoged&key=" + txt + '&users=' + current_users );
		$( '.people_seeker_response' ).html( response );
		$( '.people_seeker_response' ).css( 'display', 'block' );	
	}

	function addPeopleTransfer( user_id ){
	...
	function build_parts_number( is_edition = null ){
		...
		for (var i = 0; i < user_parts.length ; i++) {
			...
			resp += '>';//no_visible
				...
				+ ( user_parts[i]['assignment_status'] == 'canceled' ? '' : 'checked' ) + ' onchange="detect_assign_change( 0, this );"></td>'
				resp += '<td class="no_visible">' + user_parts[i]['id'] + '</td>';
				...
				resp += '<td align="right">' + user_parts[i]['assigned_transfer'] + '</td>';
				resp += '<td align="right">' 
		...
		if( $( '.assigned_users tr' ).length >= $( '#peopleNumber' ).val() || is_edition != null  ){
			...
			if( is_edition != null ){
				...
				$( '.assignations' ).append( '<button class="btn btn-success" onclick="reassignTransfer( ' 
					+ $( '#transfer_id' ).val() + ' );">Guardar Reasignación</button>' );
			}
		}
	}
...
//reasignar usuario - usuario
	...
	function detect_assign_change( type, obj ){
		if( type == 0 ){
			assign_has_changed = 1;
			$( obj ).parent('td').parent('tr').css( 'background-color', ( $( obj ).prop( 'checked' ) ?  '' : 'rgba(225, 0, 0, .3)' ) );
			//$( obj ).parent('td').parent('tr').css( 'color', ( $( obj ).prop( 'checked' ) ?  '' : 'silver' ) );
